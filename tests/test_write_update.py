...
class TestUpdateBasic:
    ...
    def test_update_with_expression(self, make_write_catalog):
        """Update using a Pandas expression (computed value)."""
        cat = make_write_catalog()
        df = pd.DataFrame({
            "a": [1, 2, 3, 4, 5],
            "b": ["one", "two", "three", "four", "five"],
    ...
    def test_update_all_rows(self, make_write_catalog):
        ...
        updated = update_ducklake(
            cat.metadata_path, "test", {"b": "ALL"}, True
        )
        assert updated == 3
...
# ---------------------------------------------------------------------------
# DuckDB interop: write with pandas, update with pandas, read with DuckDB
# ---------------------------------------------------------------------------
...
class TestUpdateEdgeCases:
    ...
    def test_scan_after_update(self, make_write_catalog):
        """read_ducklake with filter after update."""
        cat = make_write_catalog()
        df = pd.DataFrame({"a": list(range(10)), "b": ["x"] * 10})
        write_ducklake(df, cat.metadata_path, "test", mode="error")

        update_ducklake(
            cat.metadata_path, "test", {"b": "UPDATED"}, lambda df: df["a"] >= 5
        )

        result = (
            read_ducklake(cat.metadata_path, "test")
            .pipe(lambda df: df[df["b"] == "UPDATED"])
