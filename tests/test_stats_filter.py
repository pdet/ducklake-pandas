"""Filter pushdown and statistics tests for ducklake-pandas."""
...
class TestMultiFileFilterPushdown:
    ...
    def test_integer_filter_multi_file(self, ducklake_catalog):
        ...
        result = (
            ...
            .pipe(lambda df: df[df["i"] == 1050])
            
        )
        assert result.shape[0] == 1
        assert result["i"].tolist() == [1050]
    ...
    def test_integer_range_filter(self, ducklake_catalog):
        ...
        result = (
            read_ducklake(cat.metadata_path, "test")
            .pipe(lambda df: df[(df["i"] >= 1000) & (df["i"] < 1100)])
            
        )
        assert result.shape[0] == 100
        assert sorted(result["i"].tolist()) == list(range(1000, 1100))
        assert all(label == "b" for label in result["label"].tolist())
    ...
    def test_integer_greater_than(self, ducklake_catalog):
        ...
        result = (
            read_ducklake(cat.metadata_path, "test")
            .pipe(lambda df: df[df["i"] > 5000])
            
        )
        assert result.shape[0] == 99
        assert sorted(result["i"].tolist()) == list(range(5001, 5100))
    ...
    def test_integer_less_than(self, ducklake_catalog):
        ...
        result = (
            read_ducklake(cat.metadata_path, "test")
            .pipe(lambda df: df[df["i"] < 100])
            
        )
        assert result.shape[0] == 100
        assert sorted(result["i"].tolist()) == list(range(0, 100))
        assert all(label == "a" for label in result["label"].tolist())
...
class TestMultiTypeFilter:
    ...
    def test_date_filter(self, ducklake_catalog):
        ...
        result = (
            read_ducklake(cat.metadata_path, "test")
            .pipe(lambda df: df[df["d"] == target_date])
            
        )
        assert result.shape[0] == 1
        assert result["d"].tolist() == [target_date]
        # id = 100 + 14 (June 1 + 14 days = June 15)
        assert result["id"].tolist() == [114]
    ...
    def test_varchar_filter(self, ducklake_catalog):
        ...
        result = (
            read_ducklake(cat.metadata_path, "test")
            .pipe(lambda df: df[df["s"] >= "m"])
            
        )
        # Should include batch 2 (m*) and batch 3 (z*), i.e. 200 rows
        assert result.shape[0] == 200
        assert all(s >= "m" for s in result["s"].tolist())
        # Verify that no 'a' prefixed strings are present
        assert all(not s.startswith("a") for s in result["s"].tolist())
    ...
    def test_boolean_filter(self, ducklake_catalog):
        ...
        result = (
            read_ducklake(cat.metadata_path, "test")
            .pipe(lambda df: df[df["b"] == True])  # noqa: E712
            
        )
        assert result.shape[0] == 50
        assert all(b is True for b in result["b"].tolist())
        # Verify these are from batch 1 (ids 0-49)
        assert sorted(result["id"].tolist()) == list(range(0, 50))
...
class TestComplexPredicates:
    ...
    def test_or_filter(self, ducklake_catalog):
        ...
        result = (
            read_ducklake(cat.metadata_path, "test")
            .pipe(lambda df: df[(df["i"] < 10) | (df["i"] > 5090)])
            
        )
        values = sorted(result["i"].tolist())
    ...
    def test_in_filter(self, ducklake_catalog):
        ...
        result = (
            read_ducklake(cat.metadata_path, "test")
            .pipe(lambda df: df[df["i"].isin([5, 1005, 5005])])
            
        )
        assert result.shape[0] == 3
        assert sorted(result["i"].tolist()) == [5, 1005, 5005]
    ...
    def test_combined_and_or(self, ducklake_catalog):
        ...
        result = (
            read_ducklake(cat.metadata_path, "test")
            .pipe(lambda df: df[(df["i"] > 50) & (df["i"] < 1050)])
    ...
    def test_not_equal_filter(self, ducklake_catalog):
        ...
        result = (
            read_ducklake(cat.metadata_path, "test")
            .pipe(lambda df: df[df["i"] != 5])
            
        )
        assert result.shape[0] == 9
        assert sorted(result["i"].tolist()) == [0, 1, 2, 3, 4, 6, 7, 8, 9]
...
class TestNullFiltering:
    ...
    def test_is_not_null_filter(self, ducklake_catalog):
        ...
        result = (
            read_ducklake(cat.metadata_path, "test")
            .pipe(lambda df: df[df["b"].notna()])
            
        )
        # 50 from batch 1 + 25 odd values from batch 2
        assert result.shape[0] == 75
        assert all(b is not None for b in result["b"].tolist())
...
class TestTopNPruning:
    ...
    def test_sort_limit(self, ducklake_catalog):
        ...
        result = (
            read_ducklake(cat.metadata_path, "test")
            .sort_values("i", ascending=False).reset_index(drop=True)
            .head(5)
            
        )
        assert result.shape[0] == 5
        assert result["i"].tolist() == [9099, 9098, 9097, 9096, 9095]
    ...
    def test_sort_limit_ascending(self, ducklake_catalog):
        ...
        assert result.shape[0] == 5
        assert result["i"].tolist() == [0, 1, 2, 3, 4]
