...
class TestMergeDuckDBInterop:
    ...
    def test_duckdb_write_pandas_merge_duckdb_reads(self, make_write_catalog):
        ...
        source = pd.DataFrame({
            "id": pd.Series([2, 3, 4], dtype="Int32"),
            "val": ["TWO", "THREE", "FOUR"],
        })
        updated, inserted = merge_ducklake(
            cat.metadata_path, "test", source, "id",
...
class TestMergeEdgeCases:
    ...
    def test_scan_after_merge(self, make_write_catalog):
        """read_ducklake with filter after merge."""
        cat = make_write_catalog()
        df = pd.DataFrame({"id": list(range(1, 6)), "val": ["a"] * 5})
        write_ducklake(df, cat.metadata_path, "test", mode="error")

        source = pd.DataFrame({"id": [3, 6], "val": ["MERGED", "MERGED"]})
        merge_ducklake(
            cat.metadata_path, "test", source, "id",
            when_matched_update=True,
        )
        ...
        result = (
            ...
            .pipe(lambda df: df[df["val"] == "MERGED"])
            
            .sort_values(["id"]).reset_index(drop=True)
        )
        assert result["id"].tolist() == [3, 6]
