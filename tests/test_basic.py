"""Basic tests for ducklake-pandas scan and read operations."""
...
class TestBasicScan:
    """Test basic read_ducklake and read_ducklake functionality."""
    ...
    def test_basic_read(self, ducklake_catalog):
        ...
        expected = pd.DataFrame(
            {"a": pd.Series([1, 2], dtype="Int32"), "b": ["hello", "world"]}
        )
    ...
    def test_read_with_column_selection(self, ducklake_catalog):
        ...
        expected = pd.DataFrame(
            {"a": pd.Series([1], dtype="Int32"), "c": [3.14]}
        )
        assert_frame_equal(result, expected, check_dtype=False)
    ...
    def test_multiple_inserts(self, ducklake_catalog):
        ...
        assert result.sort_values(["a"]).reset_index(drop=True)["a"].tolist() == [1, 2, 3]
    ...
    def test_lazy_filter(self, ducklake_catalog):
        ...
        result = read_ducklake(cat.metadata_path, "test").pipe(lambda df: df[df["a"] > 95])
        assert sorted(result["a"].tolist()) == [96, 97, 98, 99]
    ...
    def test_lazy_select(self, ducklake_catalog):
        ...
        result = read_ducklake(cat.metadata_path, "test")[["a", "c"]]
        assert list(result.columns) == ["a", "c"]
        assert tuple(result.iloc[0]) == (1, 3.14)
