"""Tests for ducklake-pandas DELETE support (Phase 5e)."""
...
class TestDeleteBasic:
    ...
    def test_delete_all_rows(self, make_write_catalog):
        ...
        deleted = delete_ducklake(cat.metadata_path, "test", True)
        assert deleted == 3
...
# ---------------------------------------------------------------------------
# DuckDB interop: write with pandas, delete with pandas, read with DuckDB
# ---------------------------------------------------------------------------
...
class TestDeleteDuckDBInterop:
    ...
    def test_delete_all_duckdb_reads(self, make_write_catalog):
        ...
        delete_ducklake(cat.metadata_path, "test", True)
...
class TestDeleteEdgeCases:
    ...
    def test_scan_after_delete(self, make_write_catalog):
        """read_ducklake with filter after delete."""
        cat = make_write_catalog()
        ...
        result = (
            ...
            .pipe(lambda df: df[df["a"] >= 75])
            
        )
        assert sorted(result["a"].tolist()) == list(range(75, 100))
