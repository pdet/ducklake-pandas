"""Tests for partitioned writes in ducklake-pandas."""
...
class TestSetPartitionedBy:
    ...
    def test_set_partitioned_by_nonexistent_column(self, make_write_catalog):
        cat = make_write_catalog()
        create_ducklake_table(
            cat.metadata_path, "test", {"a": "Int64"},
        )
        with pytest.raises(ValueError, match="not found"):
    ...
    def test_set_partitioned_by_duckdb_reads(self, make_write_catalog):
        """DuckDB can read the partition spec created by ducklake-pandas."""
        cat = make_write_catalog()
...
class TestPartitionedInsert:
    ...
    def test_partitioned_insert_multi_column(self, make_write_catalog):
        ...
        df = pd.DataFrame({"a": [1, 2, 3], "b": ["x", "y", "x"], "c": [10, 20, 10]})
        write_ducklake(df, cat.metadata_path, "test", mode="append")

        result = read_ducklake(cat.metadata_path, "test").sort_values(["a"]).reset_index(drop=True)
        assert result["a"].tolist() == [1, 2, 3]
        assert result["b"].tolist() == ["x", "y", "x"]
        assert result["c"].tolist() == [10, 20, 10]
    ...
    def test_partitioned_insert_column_stats(self, make_write_catalog):
        ...
        df = pd.DataFrame({"a": [1, 5, 3], "b": ["x", "x", "y"]})
        write_ducklake(df, cat.metadata_path, "test", mode="append")
    ...
    def test_partitioned_insert_multiple_batches(self, make_write_catalog):
        ...
        write_ducklake(
            pd.DataFrame({"a": [3, 4], "b": ["x", "z"]}),
            cat.metadata_path, "test", mode="append",
        )

        result = read_ducklake(cat.metadata_path, "test").sort_values(["a"]).reset_index(drop=True)
        assert result["a"].tolist() == [1, 2, 3, 4]
        assert result["b"].tolist() == ["x", "y", "x", "z"]

    def test_partitioned_insert_integer_partition(self, make_write_catalog):
        """Partition on an integer column."""
        cat = make_write_catalog()
        create_ducklake_table(
            cat.metadata_path, "test", {"a": "Int64", "b": "Int64"},
        )
        alter_ducklake_set_partitioned_by(cat.metadata_path, "test", ["b"])

        df = pd.DataFrame({"a": [1, 2, 3], "b": [100, 200, 100]})
        write_ducklake(df, cat.metadata_path, "test", mode="append")
...
class TestPartitionedFilter:
    ...
    def test_filter_on_partition_column(self, make_write_catalog):
        ...
        result = lf.pipe(lambda df: df[df["b"] == "x"]).sort_values(["a"]).reset_index(drop=True)
    ...
    def test_filter_on_non_partition_column(self, make_write_catalog):
        ...
        df = pd.DataFrame({"a": [1, 5, 3, 2], "b": ["x", "x", "y", "y"]})
        write_ducklake(df, cat.metadata_path, "test", mode="append")

        lf = read_ducklake(cat.metadata_path, "test")
        result = lf.pipe(lambda df: df[df["a"] > 2]).sort_values(["a"]).reset_index(drop=True)
        assert result["a"].tolist() == [3, 5]
...
class TestPartitionedDuckDBInterop:
    """DuckDB can read partitioned data written by ducklake-pandas."""

    def test_duckdb_reads_pandas_partitioned_insert(self, make_write_catalog):
        ...
        assert sorted(pdf["b"].tolist()) == ["x", "x", "y"]

    def test_duckdb_reads_pandas_multi_partition(self, make_write_catalog):
    ...
    def test_duckdb_writes_pandas_reads_partitioned(self, make_write_catalog):
        """DuckDB creates partitioned table, ducklake-pandas writes to it."""
        cat = make_write_catalog()
        ...
        # Insert via ducklake-pandas (cast to Int32 to match DuckDB's INTEGER)
        df = pd.DataFrame({"a": pd.Series([1, 2, 3], dtype="Int32"), "b": ["x", "y", "x"]})
        write_ducklake(df, cat.metadata_path, "test", mode="append")
    ...
    def test_duckdb_insert_then_pandas_insert_partitioned(self, make_write_catalog):
        """DuckDB inserts data, then ducklake-pandas appends to same partitioned table."""
        cat = make_write_catalog()
        ...
        # Append via ducklake-pandas (cast to Int32 to match DuckDB's INTEGER)
        df = pd.DataFrame({"a": pd.Series([3, 4], dtype="Int32"), "b": ["x", "z"]})
        write_ducklake(df, cat.metadata_path, "test", mode="append")
...
class TestPartitionedRoundTrip:
    """Write and read partitioned data with ducklake-pandas."""
    ...
    def test_roundtrip_basic(self, make_write_catalog):
        ...
        assert result["category"].tolist() == ["a", "b", "a", "b", "c", "a", "d"]
        assert result["value"].tolist() == [10.0, 20.0, 30.0, 40.0, 50.0, 60.0, 70.0]
    ...
    def test_roundtrip_time_travel(self, make_write_catalog):
        ...
        result_old = result_old.sort_values(["a"]).reset_index(drop=True)
        assert result_old["a"].tolist() == [1, 2]


# ---------------------------------------------------------------------------
# DELETE on partitioned table (written by pandas)
# ---------------------------------------------------------------------------


class TestPartitionedDelete:
    """Test DELETE on partitioned tables written by ducklake-pandas."""
    ...
    def test_delete_from_partitioned(self, make_write_catalog):
        ...
        deleted = delete_ducklake(cat.metadata_path, "test", lambda df: df["a"] == 1)
        assert deleted == 1
...
# ---------------------------------------------------------------------------
# UPDATE on partitioned table (written by pandas)
# ---------------------------------------------------------------------------


class TestPartitionedUpdate:
    """Test UPDATE on partitioned tables written by ducklake-pandas."""
    ...
    def test_update_non_partition_column(self, make_write_catalog):
        ...
        updated = update_ducklake(
            cat.metadata_path, "test", {"a": 10}, lambda df: df["a"] == 1
        )
        assert updated == 1
