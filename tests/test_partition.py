...
class TestPartition:
    ...
    def test_partitioned_filter(self, ducklake_catalog):
        ...
        result = lf.pipe(lambda df: df[df["b"] == "x"])
    ...
    def test_partitioned_with_stats(self, ducklake_catalog):
        ...
        lf = read_ducklake(cat.metadata_path, "test")
        result = lf.pipe(lambda df: df[df["a"] > 2])
        result = result.sort_values(["a"]).reset_index(drop=True)
...
class TestPartitionAdvanced:
    ...
    def test_filter_on_each_partition_key(self, ducklake_catalog):
        ...
        # Filter on b only
        lf = read_ducklake(cat.metadata_path, "test")
        result_b = lf.pipe(lambda df: df[df["b"] == "x"]).sort_values(["a"]).reset_index(drop=True)
        assert result_b["a"].tolist() == [1, 3]
        assert result_b["b"].tolist() == ["x", "x"]

        # Filter on c only
        lf = read_ducklake(cat.metadata_path, "test")
        result_c = lf.pipe(lambda df: df[df["c"] == 10]).sort_values(["a"]).reset_index(drop=True)
        assert result_c["a"].tolist() == [1, 3, 5]
        assert result_c["c"].tolist() == [10, 10, 10]

        # Filter on d only
        lf = read_ducklake(cat.metadata_path, "test")
        result_d = lf.pipe(lambda df: df[df["d"] == "p"]).sort_values(["a"]).reset_index(drop=True)
