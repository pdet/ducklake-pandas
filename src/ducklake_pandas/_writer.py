...
def _stat_value_to_str(value: Any, col_type: str) -> str | None:
...
def _contains_nan(series: pd.Series) -> bool:
    """Check if a pandas Series contains NaN values (float NaN, not NA/None)."""
    try:
        if series.dtype.kind == 'f':
            return bool(np.isnan(series.dropna()).any())
        return False
    except Exception:
        return False
...
class DuckLakeCatalogWriter:
    ...
    def _insert_inlined_rows(
        ...
        for i, row in enumerate(df.itertuples(index=False)):
            ...
            for j, col_name in enumerate(col_names):
                ...
                row_vals.append(self._serialize_value(val, col_types[col_name]))
    ...
    def _delete_inlined_rows(
        ...
        for (tbl_name,) in inlined_tables:
            ...
            inline_df = pd.DataFrame(data)

            # Evaluate predicate
            mask = predicate(inline_df)
            to_delete = [row_ids[i] for i, v in enumerate(mask.tolist()) if v]
    ...
    def _flatten_schema(
        ...
        """Flatten a schema dict into column definitions.

        Parameters
        ----------
        schema_dict
            Mapping of column name to DuckDB type string (e.g. {"id": "int64", "name": "varchar"}).
        """
        defs: list[_ColumnDef] = []
        col_id = start_column_id
        order = start_order

        for name, dtype_str in schema_dict.items():
            this_id = col_id
            col_id += 1
    ...
    def _schema_from_dataframe(self, df: pd.DataFrame) -> dict[str, str]:
        """Extract DuckDB type strings from a pandas DataFrame."""
        result: dict[str, str] = {}
        for col in df.columns:
            result[col] = pandas_dtype_to_duckdb(df[col].dtype)
        return result
    ...
    def create_table(
        ...
        *,
        ...
        col_defs = self._flatten_schema(schema)
        for cd in col_defs:
            con.execute(
                ...
                [
                    cd.column_id, new_snap, table_id, cd.column_order,
                    cd.column_name, cd.column_type, cd.nulls_allowed, cd.parent_column,
                ],
            )
        ...
        self._record_change(new_snap, f'created_table:"{safe_schema}"."{safe_table}"')
    ...
    def create_table_with_data(
        ...
        *,
        ...
        """Create a new table and insert data in a single snapshot. Returns the new snapshot ID."""
        con = self._connect()
        snap_id, schema_ver, next_cat_id, next_file_id = self._get_latest_snapshot()
        ...
        schema_dict = self._schema_from_dataframe(df)
        ...
        new_snap = self._create_snapshot(new_schema_ver, new_next_cat_id, new_next_file_id)
        ...
        if has_data:
            if inline_data:
                ...
                self._update_table_column_stats(table_id, top_level_cols, col_stats)
            else:
                base = self.data_path
                if schema_path_rel:
    ...
    def _compute_file_column_stats(
        ...
        for col_id, col_name, col_type, _parent in columns:
            ...
            try:
                ...
                if t in stat_types and null_count < n:
                    non_null = series.dropna()
                    if len(non_null) > 0:
                        raw_min = non_null.min()
                        raw_max = non_null.max()
                        min_val = _stat_value_to_str(raw_min, col_type)
                        max_val = _stat_value_to_str(raw_max, col_type)
    ...
    def _merge_stat_value(
        ...
        if new is None:
            return existing
        try:
            from ducklake_pandas._stats import _parse_stat_value
            pandas_type = duckdb_type_to_pandas(col_type)
            e_val = _parse_stat_value(existing, pandas_type)
            n_val = _parse_stat_value(new, pandas_type)
            if e_val is None:
                return new
            if n_val is None:
                return existing
    ...
    def _register_data_file(
        ...
        con.execute(
            ...
            [
                data_file_id, table_id, new_snap, rel_path, True,
    ...
    def _register_file_column_stats(
        ...
        for col_id, _col_name, value_count, null_count, min_val, max_val, nan_int in col_stats:
            con.execute(
                ...
                "VALUES (?, ?, ?, 0, ?, ?, ?, ?, ?, NULL)",
                [
                    data_file_id, table_id, col_id, value_count, null_count,
                    min_val, max_val, nan_int,
                ],
            )
    ...
    def _insert_partitioned(
        ...
        group_list: list[tuple[tuple, pd.DataFrame]] = []
        for group_key, group_df in df.groupby(part_col_names, sort=False):
    ...
    def overwrite_data(
        ...
        *,
        ...
        """Overwrite all data in a table. Returns the new snapshot ID."""
        con = self._connect()
        snap_id, schema_ver, next_cat_id, next_file_id = self._get_latest_snapshot()
    ...
    def _get_active_delete_positions(
        ...
        """Return row positions that are deleted for a data file."""
        con = self._connect()
        rows = con.execute(
        ...
        for del_path, del_is_rel in rows:
            ...
            try:
                del_df = pd.read_parquet(abs_del)
    ...
    def _read_active_data_file(
        ...
        if deleted_positions:
            mask = ~df.index.isin(deleted_positions)
    ...
    def _write_cumulative_delete_file(
        ...
        """Write a cumulative delete file combining existing + new positions."""
        con = self._connect()
        ...
        *,
        ...
        if data_files:
            ...
            for data_file_id, rel_path, path_is_rel, record_count, row_id_start in data_files:
                ...
                raw_df = pd.read_parquet(abs_path)
                ...
                positions = [
                    i for i, v in enumerate(mask.tolist())
    ...
    def _get_inlined_matched_rows(
        ...
        for (tbl_name,) in inlined_tables:
            ...
            inline_df = pd.DataFrame(data)
            mask = predicate(inline_df)
            matched = inline_df[mask].reset_index(drop=True)
            if len(matched) > 0:
                all_matched.append(matched)

        if not all_matched:
            return None
        return pd.concat(all_matched, ignore_index=True) if len(all_matched) > 1 else all_matched[0]
    ...
    def update_data(
        ...
        *,
        ...
        for data_file_id, rel_path, path_is_rel, record_count, row_id_start in data_files:
            ...
            if positions:
                pending_deletes.append((data_file_id, abs_path, positions))
                active_matched = raw_df.iloc[positions].reset_index(drop=True)
        ...
        all_matched = pd.concat(matched_dfs, ignore_index=True)

        # Apply updates
        updated_df = all_matched.copy()
        for col_name, value in updates.items():
            if callable(value):
                updated_df[col_name] = value(updated_df)
            else:
                updated_df[col_name] = value
        ...
        if part_id is not None:
            ...
            group_list: list[tuple[tuple, pd.DataFrame]] = []
            for gk, gdf in updated_df.groupby(part_col_names, sort=False):
        ...
        if part_id is not None and group_list:
            ...
            for gk, gdf in group_list:
                ...
                self._register_file_column_stats(current_data_file_id, table_id, cs)
    ...
    def _read_all_inlined_active_rows(
        ...
        """Read all active inlined rows as a DataFrame."""
        con = self._connect()
        try:
            inlined_tables = con.execute(
                "SELECT table_name FROM ducklake_inlined_data_tables "
        ...
        if not all_dfs:
            return None
        return pd.concat(all_dfs, ignore_index=True) if len(all_dfs) > 1 else all_dfs[0]
    ...
    def _build_key_match_predicate(
        ...
        """Build a predicate that matches rows whose key columns are in *matched_keys_df*."""
        if len(on) == 1:
            key_col = on[0]
            key_values = set(matched_keys_df[key_col].tolist())
            return lambda df: df[key_col].isin(key_values)

        # Multiple key columns: merge-based match
        key_tuples = set(matched_keys_df[on].apply(tuple, axis=1).tolist())
        def pred(df: pd.DataFrame) -> pd.Series:
            return df[on].apply(tuple, axis=1).isin(key_tuples)
        return pred
    ...
    def merge_data(
        ...
        *,
        ...
        source_keys = source_df[on].drop_duplicates()

        pending_deletes: list[tuple[int, str, list[int]]] = []
        matched_target_dfs: list[pd.DataFrame] = []
        all_target_key_dfs: list[pd.DataFrame] = []
        ...
        for data_file_id, rel_path, path_is_rel, _rc, _rid in data_files:
            ...
            file_df = raw_df.copy()
            file_df["__merge_idx__"] = range(len(file_df))
            if already_deleted:
                file_df = file_df[~file_df["__merge_idx__"].isin(already_deleted)]

            active_df = file_df.drop(columns=["__merge_idx__"])
            all_target_key_dfs.append(active_df[on])

            if when_matched_update is not None:
                # Semi join: find rows in file_df that match source_keys
                merged = file_df.merge(source_keys, on=on, how="inner")
                if len(merged) > 0:
                    positions = sorted(merged["__merge_idx__"].tolist())
                    pending_deletes.append((data_file_id, abs_path, positions))
                    matched_target_dfs.append(merged.drop(columns=["__merge_idx__"]))

        inlined_matched_df: pd.DataFrame | None = None
        if inlined_count > 0:
            inlined_df = self._read_all_inlined_active_rows(
                table_id, snap_id, columns,
            )
            if inlined_df is not None and len(inlined_df) > 0:
                all_target_key_dfs.append(inlined_df[on])
                if when_matched_update is not None:
                    inlined_matched = inlined_df.merge(source_keys, on=on, how="inner")
        ...
        if when_not_matched_insert:
            if all_target_key_dfs:
                all_target_keys = pd.concat(all_target_key_dfs, ignore_index=True).drop_duplicates()
                # Anti join
                merged = source_df.merge(all_target_keys, on=on, how="left", indicator=True)
                unmatched_source = merged[merged["_merge"] == "left_only"].drop(columns=["_merge"])
            else:
                unmatched_source = source_df
        else:
            unmatched_source = source_df.iloc[0:0]
        ...
        if when_matched_update is not None and matched_target_dfs:
            all_matched = pd.concat(matched_target_dfs, ignore_index=True)
            if when_matched_update is True:
                updated_rows = all_matched[on].merge(source_df, on=on, how="inner")
                rows_to_insert_parts.append(updated_rows[col_names])
            elif isinstance(when_matched_update, dict):
                updated = all_matched.copy()
                for col_name, value in when_matched_update.items():
                    if callable(value):
                        updated[col_name] = value(updated)
                    else:
                        updated[col_name] = value
                rows_to_insert_parts.append(updated)

        if total_inserted > 0:
            rows_to_insert_parts.append(unmatched_source[col_names])

        if not rows_to_insert_parts:
            return (0, 0)

        insert_df = (
            pd.concat(rows_to_insert_parts, ignore_index=True)
        ...
        if part_id is not None and n_data_files > 0:
            part_cols = self._get_partition_columns(part_id, table_id)
            col_id_to_name2: dict[int, str] = {c[0]: c[1] for c in columns}
            part_col_names2 = [col_id_to_name2[cid] for _, cid, _ in part_cols]
            part_key_indices2 = [ki for ki, _, _ in part_cols]
            group_list2: list[tuple[tuple, pd.DataFrame]] = []
            for gk, gdf in insert_df.groupby(part_col_names2, sort=False):
                if not isinstance(gk, tuple):
                    gk = (gk,)
                group_list2.append((gk, gdf))
            n_data_files = len(group_list2)
        else:
            part_col_names2 = []
            part_key_indices2 = []
            group_list2 = []
        ...
        if inlined_matched_df is not None and len(inlined_matched_df) > 0:
            key_pred = self._build_key_match_predicate(
                on, inlined_matched_df[on].drop_duplicates(),
            )
            self._delete_inlined_rows(
        ...
        if part_id is not None and group_list2:
            total_file_size = 0
            current_data_file_id = first_data_file_id
            current_row_id = row_id_start

            for gk, gdf in group_list2:
                partition_values = [str(v) for v in gk]
                hive_subdir = self._build_hive_path(part_col_names2, partition_values)
                partition_dir = os.path.join(base, hive_subdir)
                os.makedirs(partition_dir, exist_ok=True)
                ...
                self._register_partition_values(
                    ...
                    part_key_indices2, partition_values,
                )
                cs = self._compute_file_column_stats(gdf, columns)
            ...
            file_name = f"ducklake-{_uuid7()}.parquet"
            file_path = os.path.join(base, file_name)
            insert_df.to_parquet(file_path, index=False)
    ...
    def add_column(
        ...
        *,
        ...
        ----------
        dtype
            DuckDB type string (e.g. "int64", "varchar", "boolean").
        """
        con = self._connect()
        snap_id, schema_ver, next_cat_id, next_file_id = self._get_latest_snapshot()
    ...
    def create_schema(
        ...
        """Create a new schema. Returns the new schema_id."""
        con = self._connect()
        snap_id, schema_ver, next_cat_id, next_file_id = self._get_latest_snapshot()
